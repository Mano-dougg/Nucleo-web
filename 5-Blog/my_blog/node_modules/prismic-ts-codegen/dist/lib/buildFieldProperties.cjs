  ...
  switch (args.field.type) {
    ...
    case "Group": {
      ...
      if (args.path[0].model && "type" in args.path[0].model && args.path[0].model.type === "SharedSlice") {
        ...
        itemName = buildTypeName.buildTypeName(slicePathPart.name, "Slice", variationPathPart.name, zonePathPart.name, args.name, "Item");
      } else {
        itemName = buildTypeName.buildTypeName(args.path[0].name, "Document", "Data", args.name, "Item");
      }
      const path = [
        ...args.path,
        {
